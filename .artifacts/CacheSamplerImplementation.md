type: EXPRESSION
#expression: biome_place2(x,z)
#expression: round((elevation(x,z)*10))/10 #24 s
#expression: round(continents(x,z)) #4 - 7 s, 29 s with cache?
expression: round(continentsCached(x,z)) 40 s
#expression: round(temperature(x,z))  #~30 s
#expression: round(elevation(x,z))
#expression: x+(y*.0001)^2+z
#expression: cell_test(x,z)
#expression: cell_test(x,z) #2 S
#expression: precipitation(x,z) #8 s
#expression: round(temperature(x,z)*elevation(x,z)) 68 s
#expression: cell_test2(x,z)
...
samplers:
  cell_test: &cell_test
    ...
    return: NoiseLookup   
    jitter: 1.2
    distance: EuclideanSq #Dflt: EuclideanSq
    #salt: 7777.0
    #NoiseSampler: continents
    NoiseSampler: OPEN_SIMPLEX_2
  biome_place: &biome_place
    dimensions: 2
    type: DOMAIN_WARP
    amplitude: 20
    warp:
    ...
    sampler:
      ...
      sampler:
        ...
        sampler:
          ...
          distance: Hybrid
          salt: 7777.0
  biome_place2: *biome_place
# --- RESOLVED SAMPLERS BELOW ---
  cell_test:
    dimensions: 2
    type: CELLULAR
    frequency: 0.005
  ...
  elevation: &elevation
    ...
    samplers:
      ...
      riverTerrainErosion:
        dimensions: 2
        type: EXPRESSION
        expression: "herp(riverDistance(x, z) + herp(erosionStrength(x, z), 0.6, erosionMaxDistance, 0.4, 0),\n  0, erosionMax,\n\
            ...
            sampler:
              ...
              sampler:
                ...
                sampler:
                  ...
                  salt: 1.0
                  frequency: 0.0002
  oceanElevationCached:
    dimensions: 2
    type: CACHE
    sampler: *oceanElevation
  rawPrecipitation:
    dimensions: 2
    type: EXPRESSION
